"""EvaluatorAgent — multi-turn Socratic evaluation orchestrator.

Runs the evaluation pipeline on the shared LiteLLM-backed `evaluator` model
group (see ``utils.llm_client`` / ``litellm/config.yaml``). Sessions and their
multi-turn state are held in-memory keyed by session_id; the worker layer
(``workers.evaluator_worker``) drives a single step per job. Rehydration across
restarts is handled separately (EVAL-02 state store).
"""
...
class EvaluatorAgent:
    ...
    def __init__(self, llm_client: Optional[GeminiClient] = None, require_llm: bool = True):
        ...
        Sessions live in-memory (``self.sessions``); no database is used.
        Rehydration across worker restarts is supplied by the EVAL-02 state
        store, not by this agent.

        Args:
            llm_client: Optional pre-initialized GeminiClient shim over the
                shared LiteLLM ``evaluator`` model group.
            require_llm: If True (default), builds the LLM client lazily.
                If False, the agent runs with ``self.llm = None`` and falls
                back to local concept-coverage scoring.
        """
        if llm_client is not None:
            self.llm = llm_client
            logger.info("Using provided GeminiClient for evaluation")
        elif require_llm:
            self.llm = GeminiClient()
            logger.info("Using shared LiteLLM 'evaluator' model group")
        else:
            self.llm = None
            logger.info("Initialized without LLM (local concept-coverage scoring)")
