from app.services.aggregation_service import get_all_weeks_summary


def compute_trends(user_id: str) -> dict:
    """
    Analyse l'évolution des métriques sur les dernières semaines.
    Retourne des tendances et un score global de progression.
    """
    ...
    # On compare toujours la dernière semaine à la précédente
    latest   = weeks[-1]
    previous = weeks[-2]
    ...
    def trend_label(current, previous, higher_is_better=True):
        ...
        if higher_is_better:
            ...
            # Pour la fatigue, une baisse est une amélioration
            if change < -5: return "improving"
            if change >  5: return "declining"
            return "stable"
    ...
    # Score global de progression sur 100
