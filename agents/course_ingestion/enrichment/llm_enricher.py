...
if not api_key:
    raise ValueError(f"GEMINI_API_KEY not found in environment. Checked .env at: {env_path}")
...
def clean_metadata(text: str) -> str:
    ...
    text = re.sub(r'\b(Prof\.|Professor|Dr\.|Docteur|M\.|Mme|Madame|Monsieur|Mr\.|Mrs\.|Ms\.)\s+[A-Z][a-z]+(\s+[A-Z][a-z]+)*', '', text, flags=re.IGNORECASE)
    ...
    text = re.sub(r'\b(January|February|March|April|May|June|July|August|September|October|November|December|Janvier|Février|Mars|Avril|Mai|Juin|Juillet|Août|Septembre|Octobre|Novembre|Décembre)\s+\d{1,2},?\s+\d{4}\b', '', text, flags=re.IGNORECASE)
    ...
    text = re.sub(r'\b(University|Université|Institute|Institut|School|École|Ecole|College|Collège|ESPRIT|Polytechnique)\s+(of|de|d\')?[A-Z]?[a-z]*(\s+[A-Z][a-z]+)*', '', text, flags=re.IGNORECASE)
    ...
    text = re.sub(r'\b(Chapter|Chapitre|Section|Part|Partie)\s+\d+[:\.]?\s*', '', text, flags=re.IGNORECASE)
    
    # Remove "Plan module" and similar course structure words
    text = re.sub(r'\b(Plan\s+(module|cours|de\s+cours)|Module\s+plan|Course\s+outline)\b', '', text, flags=re.IGNORECASE)
...
"""
    ...
    try:
        ...
        if "key_concepts" in data and isinstance(data["key_concepts"], list):
            data["key_concepts"] = [clean_metadata(c) for c in data["key_concepts"] if c]
