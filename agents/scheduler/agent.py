...
class SchedulerAgent:
    ...
    def build_schedule(
        ...
        if context.hourly_focus_profile is None and context.user_id:
            try:
                from services.signal_processing_service.repository import (
                    SignalRepository,
                )
    ...
    def _schedule_day(
        ...
        while (
            ...
            if not self._prerequisites_met(task, scheduled_task_ids, title_to_id):
                logger.warning(
                    "scheduler_prereq_unmet", extra={"task_id": task.task_id}
                )
                skipped_tasks.add(task.task_id)
    ...
    def _apply_fallback_schedule(
        ...
        while (
            ...
            current_time = end_time + timedelta(minutes=self.FALLBACK_BREAK_MINUTES)
